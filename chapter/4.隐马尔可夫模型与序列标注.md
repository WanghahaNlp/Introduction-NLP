...
**序列标注**指的是给定一个序列 ![](https://github.com/WanghahaNlp/Introduction-NLP/blob/master/img/%2020-2-8_0-41-12.gif)，找出序列中每个元素对应标签 ![](https://github.com/WanghahaNlp/Introduction-NLP/blob/master/img/%2020-2-8_0-43-48.gif) 的问题。其中，y 所有可能的取值集合称为**标注集**。比如，输入一个自然数序列，输出它们的奇偶性。

![](https://github.com/WanghahaNlp/Introduction-NLP/blob/master/img/%2020-2-6_10-36-13.png)

求解序列标注问题的模型一般称为**序列标注器**，通常由模型从一个标注数据集 ![](https://github.com/WanghahaNlp/Introduction-NLP/blob/master/img/%2020-2-8_0-45-19.gif) 中学习相关知识后再进行预测。再NLP问题中，x 通常是字符或词语，而 y 则是待预测的组词角色或词性等标签。中文分词、词性标注以及命名实体识别，都可以转化为序列标注问题。

1. **序列标注与中文分词**

   考虑一个字符序列(字符串) x，想象切词器真的是在拿刀切割字符串，如此，中文分词转化为标注集{切，过}的序列标注问题。

   ![](https://github.com/WanghahaNlp/Introduction-NLP/blob/master/img/%2020-2-6_10-55-28.png)

   

   **分词标注集**并非只有一种，为了捕捉汉字分别作为词语收尾(**B**egin、**E**nd)、词中(**M**iddle)以及单字成词(**S**ingle)时不同的成词概率，人们提出了{B,M,E,S}这种最流行的标注集。

   ![](https://github.com/WanghahaNlp/Introduction-NLP/blob/master/img/%2020-2-6_11-4-22.png)

   

2. **序列标注与词性标注**

   词性标注任务是一个天然的序列标注问题：x 是单词序列，y 是相应的词性序列。需要综合考虑前后的单词与词性才能决定当前单词的词性。![](https://github.com/WanghahaNlp/Introduction-NLP/blob/master/img/%2020-2-6_11-11-0.png)
...
3. **序列标注与命名实体识别**
   ...
   ![](https://github.com/WanghahaNlp/Introduction-NLP/blob/master/img/%2020-2-6_11-23-10.png)
...
1. **从马尔可夫假设到隐马尔可夫模型**
   ...
   **隐马尔可夫模型**：它的马尔可夫假设作用于状态序列，
   ...
   ![](https://github.com/WanghahaNlp/Introduction-NLP/blob/master/img/%2020-2-6_11-43-39.png)
...
2. **初始状态概率向量**
   ...
   ![](https://github.com/WanghahaNlp/Introduction-NLP/blob/master/img/%2020-2-8_0-47-45.gif)
   
   是概率分布的参数向量，称为**初始状态概率向量**。

   ![](https://github.com/WanghahaNlp/Introduction-NLP/blob/master/img/%2020-2-6_11-57-24.png)

   

   给定 π ，初始状态 Y1 的取值分布就确定了，比如采用{B,M,E,S}标注集时概率如下：
   
   ![](https://github.com/WanghahaNlp/Introduction-NLP/blob/master/img/%2020-2-8_0-49-50.gif)
...
3. **状态转移矩阵**
   ...
   ![](https://github.com/WanghahaNlp/Introduction-NLP/blob/master/img/%2020-2-8_0-51-27.gif)
...
4. **发射概率矩阵**
   ...
   ![](https://github.com/WanghahaNlp/Introduction-NLP/blob/master/img/%2020-2-8_0-52-50.gif)
...
1. **案例假设和模型构造**
   ...
   ![](https://github.com/WanghahaNlp/Introduction-NLP/blob/master/img/%2020-2-6_14-9-11.png)
...
1. **概率计算的前向算法**
   ...
   - 顺着隐马尔可夫链走，首先 t=1 时初始状态没有前驱状态，发生概率由 π 决定：
     ...
     ![](https://github.com/WanghahaNlp/Introduction-NLP/blob/master/img/%2020-2-8_0-55-17.gif)

   - 接着对 t >= 2，状态 Yt 由前驱状态 Yt-1 转移而来，转移矩阵由矩阵 A 决定：

     ![](https://github.com/WanghahaNlp/Introduction-NLP/blob/master/img/%2020-2-8_0-56-28.gif)

     所以状态序列的概率为上面式子的乘积：

     ![](https://github.com/WanghahaNlp/Introduction-NLP/blob/master/img/%2020-2-8_0-57-47.gif)

   - P(y) 我们已经求出来了，下面要求 P(x|y)

     对于每个 Yt = Si，都会“发射”一个 Xt = Oj，发射概率由矩阵 B 决定：

     ![](https://github.com/WanghahaNlp/Introduction-NLP/blob/master/img/%2020-2-8_0-59-16.gif)

   - 那么给定一个状态序列 Y，对应的 X 的概率累积形式：

     ![](https://github.com/WanghahaNlp/Introduction-NLP/blob/master/img/%2020-2-8_1-0-30.gif)

   - 最后带入联合概率公式得：
     
     ![](https://github.com/WanghahaNlp/Introduction-NLP/blob/master/img/%2020-2-8_1-1-42.gif)
...
2. **搜索状态序列的维特比算法**
   ...
   ![](https://github.com/WanghahaNlp/Introduction-NLP/blob/master/img/%2020-2-7_17-57-2.png)
   ...
   - **初始化**，t=1 时初始最优路径的备选由 N 个状态组成，它们的前驱为空。
     ...
     ![](https://github.com/WanghahaNlp/Introduction-NLP/blob/master/img/%2020-2-8_1-3-7.gif)
   ...
   - **递推**，t >= 2 时每条备选路径像贪吃蛇一样吃入一个新状态，长度增加一个单位，根据转移概率和发射概率计算花费。找出新的局部最优路径，更新 δ、ψ 两个数组。
     ...
     ![](https://github.com/WanghahaNlp/Introduction-NLP/blob/master/img/%2020-2-8_1-4-40.gif)
     
   - **终止**，找出最终时刻 δt,i 数组中的最大概率 P*，以及相应的结尾状态下标 i\*t。
   
     ![](https://github.com/WanghahaNlp/Introduction-NLP/blob/master/img/%2020-2-8_1-6-54.gif)
   
   - **回溯**，根据前驱数组 **ψ** 回溯前驱状态，取得最优路径状态下标。
   
     ![](https://github.com/WanghahaNlp/Introduction-NLP/blob/master/img/%2020-2-8_1-8-16.gif)
